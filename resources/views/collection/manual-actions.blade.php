...
@php  }@endphp
 @endforeach
    ...
    background-color: rgba(0, 0, 0, 0.4);">
                <div class="modal-content">
                    <!--new-Holap-->
                    <div class="new-holap-2" id="hide-hoalp-2">
                        <div class="container">
                            <div class="row">
...
<script src="/js/collection.js"></script>
  ...
  // Function to show the HollabTwo section and make an AJAX call
  function showHollabTwo(actionId, clientId) {
      const timestamp = Date.now(); // Simplified unique timestamp
      const previewElement = $('#preview');

      $.ajax({
          type: 'GET',
          url: "{{ route('action.preview') }}",
          data: {
              action_id: actionId,
              client_id: clientId,
              _: timestamp
          ...
          success: data => {
              ...
              if (data && previewElement.children().length > 0) {
                  console.log(actionId);
                  document.getElementById("myModal-"+actionId).style.display = "none";
  ...
  // Function to show the initial section
  function showHollab() {
    holabOne.classList.remove("d-none");
    holabTwo.classList.add("d-none");
  }

  // Ensure AJAX setup for CSRF token is done before the AJAX request
  $.ajaxSetup({
    headers: {
